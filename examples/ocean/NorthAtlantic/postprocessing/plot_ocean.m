...
%---------------------------------------------------------------------
    ...
    scale_salflux=1;
    if isfield(opts, 'scale_salflux')
        scale_salflux=opts.scale_salflux
    end
    ...
    if ( plot_salflux || plot_everything )  ...
        ...
        if isfield(add, 'SalinityFlux')
          figure(fig_ctr); fig_ctr = fig_ctr+1;
          im = reshape(add.SalinityFlux,n,m)*scale_salflux;
          im(im==0) = NaN;
          ...
          ylabel('Latitude');
          colorbar;
          % reverse begin and end color (these are taken from my_colormap)
          sze = 1e4;
          bcol = [0    0.4470    0.7410];
          ecol = [0.8500    0.3250    0.0980];
          cmap = [my_colmap(caxis,0,sze,ecol,bcol)];
          colormap(cmap)
      end
    end
    ...
    fluxes = [];
    if readFluxes
        fluxes = plot_fluxes(add, fig_ctr);
    end
